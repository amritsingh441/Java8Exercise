...
public class DateTimeExercise {
	...
	private static List<Tablet> tabletList = new ArrayList<Tablet>();
	...
	public static List<String> getExpiringTables(int months){
		return tabletList.stream()
				...
				.map(tablet -> tablet.getTabletName()).collect(Collectors.toList());
	...
	public static Map<String,List<String>> getSameYearExpiry() {
		...
		return tabletList.stream()
				...
				.collect(Collectors.groupingBy(tablet -> tablet.getManufacturer(),
						Collectors.mapping(Tablet :: getTabletName, Collectors.toList())));
	}

	public static void main(String[] args) {
		DateTimeExercise dateTimeExercise =  new DateTimeExercise();
		System.out.println("<-----------getExpiringTables method output------------------------------>");
		List<String> expiredTablets = getExpiringTables(36);
		expiredTablets.forEach(System.out::println);
		System.out.println("<------------getExpiringTabletsSorted method output----------------------->");
		List<Tablet> sortedTabletList = getExpiringTabletsSorted();
		sortedTabletList.forEach(System.out::println);
		System.out.println("<------------getTabletExpiryPeriod method output----------------------->");
		Map<String, String> tabletExpiryMap = getTabletExpiryPeriod();
		for (String str : tabletExpiryMap.keySet()) {
			System.out.println(str +" : "+ tabletExpiryMap.get(str));
		}
		System.out.println("<------------getSameYearExpiry method output----------------------->");
