...
public class StreamOperationsExercise {
	...
	static {
		...
		playerList.add(new Player("Vijay", 215, 6561, 823, "Australia"));
		playerList.add(new Player("Amrit", 121, 3700, 685, "India"));
		playerList.add(new Player("Bala", 11, 1561, 376, "WestIndies"));
		playerList.add(new Player("Entaz", 14, 2261, 190, "SriLanka"));
		playerList.add(new Player("Gayatri", 221, 11561, 923, "Australia"));
		playerList.add(new Player("Chaman", 125, 1001, 62, "SriLanka"));
		playerList.add(new Player("Bhuvan", 215, 7001, 562, "India"));
		playerList.add(new Player("Lakha", 25, 1601, 79, "WestIndies"));
		playerList.add(new Player("Kachra", 25, 3001, 62, "India"));
	...
	public static void displayPlayersForCountry(String country) {
		...
		List<String> countrySpecificPlayerList = playerList.stream()
				.filter(player -> player.getCountry().equalsIgnoreCase(country) && player.getHighestScore()>100)
	...
	public static int getAverageRunsByCountry(String countryName) {
		...
		Stream<Player> playerStream =playerList.stream().filter(players -> players.getCountry().equalsIgnoreCase(countryName));
	...
	public static List<String> getPlayerNamesSorted() {
		...
		List<String> playerNamesList = 	playerList.stream()
				.sorted(Comparator.comparing((Player p) -> p.getCountry())
	...
	public static Map<String, String> getPlayerCountry() {
		...
		Map<String, String> playerMap =	playerList.stream()
				...
				.collect(Collectors.toMap(p -> p.getPlayerName(), p -> p.getCountry()));
		return playerMap;
	...
	public static void findPlayer(String name, String country) {
		...
		Optional<Player> studOptional =	playerList.stream()
				.filter(player -> player.getPlayerName().equalsIgnoreCase(name) && player.getCountry().equalsIgnoreCase(country)).findAny();
	...
	boolean playerFound = playerList.stream().anyMatch(player -> player.getCountry().equalsIgnoreCase(country) && player.getRuns() > 10000);
	return playerFound;
